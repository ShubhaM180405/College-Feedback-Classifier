...
# ==========================
force_decisive_sentiment = False  # ✅ Recommended default for fairness and clarity
...
def classify_sentiment_chunkwise(text):
    ...
    if len(top_sentiments) > 1:
        ...
        if avg_scores:
            ...
            if len(dominant) == 1:
                if force_decisive_sentiment:
                    overall_sentiment = dominant[0]
                    sentiment_hint = f"(⚠️ Tie, leaning {dominant[0].lower()} based on confidence.)"
                else:
                    overall_sentiment = "Neutral"
                    sentiment_hint = f"(⚖️ Tie, but it leans {dominant[0].lower()} based on confidence.)"
            else:
                overall_sentiment = "Neutral"
